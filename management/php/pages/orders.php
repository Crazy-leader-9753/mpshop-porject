            ...
            success: function(response) {
                ...
                for (i = 0; i < all_data.length; i++) {
                    if (all_data[i].payment_status == "pending") {
                        ps = "<i class='fa-solid fa-triangle-exclamation text-warning fs-3 mb-2'></i><button class='btn btn-success update btn-sm px-2 py-1' oid='" + all_data[i].id + "' btn_type='payment_status'>Update</button>";
                    }
                     else 
                     {
                        ps = "<i class='fa-solid fa-check-circle text-success fs-3'></i>";
                    }
                    if (all_data[i].order_status == "pending") 
                    {
                        os = "<i class='fa-solid fa-triangle-exclamation text-warning fs-3 mb-2'></i><button class='btn btn-success update btn-sm px-2 py-1' oid='" + all_data[i].id + "' btn_type='order_status'>Update</button>";
                    } 
                    else
                     {
                        os = "<i class='fa-solid fa-check-circle text-success fs-3'></i>";
                    }
                ...
                $(".update").each(function() {
                    $(this).click(function() {
                        ...
                        $.ajax({
                            ...
                            {
                               $(element).html("<i class='fa-solid fa-check-circle text-success fs-3'></i>"); 
                            }
                            }
                        })
                    });
